...
| `KicksterStatComparison` | `kickster_stat_comparison.dart` | Linha de estatística (Figma "Statistic"): **valor da casa** (`primary`) · **rótulo** centralizado (`textSecondary`) · **valor do visitante** (`textPrimary`) + duas **lanes proporcionais** (6px, raio 12) em **monocromia** — `primary` (casa) e `primary`@35% (visitante), sem cor concorrente. `homeLabel`/`awayLabel` aceitam valores formatados (ex.: `54%`) |
| `KicksterTimeline` | `kickster_timeline.dart` | Lista de eventos em **coluna única**: badge circular 40px `surface` com borda `line` e sombra `elevation.timeline.badge` + bloco de texto com tempo (`success` 12 w600) · título (14 w600) · subtítulo (12 `textSecondary`). Em telas estreitas, o texto usa a largura toda (o espelhamento casa/visitante truncava); **o lado vai no texto** (nome do time no subtítulo), nunca só na posição/cor (WCAG 1.4.1). Cada evento é **um nó de acessibilidade** quando `semanticLabel` é informado; `onTap` abre o detalhe (`KicksterBottomSheet`) |
...
- **Public App − campeonato selecionado (`CompetitionDetailScreen`)**: cabeçalho com **organização + `CompetitionStatusBadge` + `CompetitionAttributes`** (mesmo formato do card, componente reutilizado) e **abas em pílula** (`KicksterPillTabs`): Jogos · Resultados · Classificação · Estatísticas. O **“Trocar”** no `PublicAppBar` volta à lista.
  - **Aba Jogos — padrão "Fixtures"** (Figma `34433:3347`): jogos **agrupados por data** (cabeçalho com `calendar_today_outlined` 24 + data `dd/MM/yyyy` 16/24 w500) e, por jogo, uma **linha** `surface.muted` raio **12**, padding **16×12**: `Time A (nome à direita + escudo 32)` · **pílula branca central** 96px (fundo `surface`, borda `line`, raio 12, padding 4×8, texto 12/16 w500 `primary`) com **horário** (agendado) ou **placar** (ao vivo/encerrado) · `escudo 32 + Time B`.
  - O filtro por rodada segue como `KicksterChip` (compacto) acima dos grupos.
- **Public App − tela de Jogo (`GameDetailScreen`)**: hub do confronto com **duas abas** — **Estatísticas** (`KicksterStatComparison`, no máximo **6 linhas** visíveis + "Ver todas"; métricas derivadas dos lances) e **Lances** (`KicksterTimeline`); cabeçalho `KicksterGameHeader` compartilhado e **troca de aba in-place** (não empilha rotas). O local fica em **linha compacta** ao fim de Estatísticas.
