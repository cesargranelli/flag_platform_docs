...
- **Public App − campeonato selecionado (`CompetitionDetailScreen`)**: **cabeçalho compacto de 1 linha** (organização + `CompetitionStatusBadge`) com os **atributos atrás de um toque** (`CompetitionAttributes` em `KicksterBottomSheet` "Sobre o campeonato" — evita repetir no hub o que o card da lista já mostrou) e **2 abas segmentadas** (`KicksterSegmentedTabs`): **Jogos | Classificação**. O botão **"Trocar"** (com rótulo) abre um `KicksterBottomSheet` com a lista de campeonatos — troca sem sair do hub. A aba **Estatísticas** sai do hub enquanto for mock (dados hardcoded).
  - **Aba Jogos** funde "Resultados": **segmento interno** `Próximos | Resultados` (`KicksterSegmentedTabs`) — Próximos = jogos não encerrados; Resultados = encerrados (com placar).
  ...
  - **Rodada**: seleção **discreta** (botão `Rodada` → bottom sheet). Default = a **próxima rodada com jogos** (ou a última com resultado, no segmento Resultados). **Uma taxonomia por vez**: com rodada escolhida, a lista fica **achatada** (sem cabeçalhos de data).
