...
- **Public App − arquitetura de informação (dock)**: `Início · Jogos · Classificação · Sobre`. A competição deixou de ser uma "tela com abas": virou um **filtro compartilhado** (`focusedCompetitionProvider`, persistido), selecionável por um único controle (`CompetitionSelector`: botão + `KicksterBottomSheet` com "Todas as competições" e a lista).
  - **Aba Jogos** (`/live`): **o Calendário** — agenda de todas as competições com **filtro por data** (`Todas as datas` + dias) **e** por **competição** (`CompetitionSelector`). Lista no padrão **Fixtures** (`FixtureRow`/`FixtureDateHeader`), cobrindo **agendados, hoje e finalizados** (exceto cancelados), agrupada por data quando o dia está em "Todas". Os controles **permanecem visíveis** em loading/erro.
  - **Aba Classificação** (`/competition`): **sem competição selecionada** mostra a **lista de campeonatos** (destino de "Ver todas") com **filtro de competição + filtro de modalidade** (`ModalitySelector`, `KicksterBottomSheet`); **com competição selecionada** mostra os segmentos **Classificação** (tabela) e **Estatísticas** (líderes — mock por enquanto). A lista fixa apenas até a primeira escolha.
  - **Rotas**: `/competition/:id/{games,results}` → **aba Jogos** filtrada; `/competition/:id/{standings,stats}` → **aba Classificação** (segmento 0 / 1); `/competitions` → lista. (A rota `/schedule` foi removida — o Calendário é a própria aba Jogos.)
