...
- **Public App − competições**: só são **listadas** as competições **em andamento** (`status = PUBLISHED`) — via `ongoingCompetitionsProvider`. Rascunhos, encerradas e desativadas não aparecem nas listagens.
- **Public App − arquitetura de informação (dock)**: `Início · Jogos · Classificação · Sobre`. A competição deixou de ser uma "tela com abas": virou um **filtro compartilhado** (`focusedCompetitionProvider`, persistido), selecionável por um único controle (`CompetitionSelector`: botão + `KicksterBottomSheet` com "Todas as competições" e a lista).
  - **Aba Jogos** (`/live`): superfície **única** para **Hoje · Agendados · Resultados** (`KicksterSegmentedTabs`, com o `CompetitionSelector` acima). Substitui a antiga "Jogos do Dia" **e** a aba de jogos do campeonato — fim da duplicação e das taxonomias concorrentes.
    - **Segmentos**: **Hoje** = jogos do dia (qualquer status); **Agendados** = `scheduled`/`open` (agrupados por data); **Resultados** = `finished` (mais recentes primeiro).
    - **Padrão "Fixtures"** (Figma `34433:3347`), reutilizável (`FixtureRow` + `FixtureDateHeader`): por jogo, uma **linha** `surface.muted` raio **12**, padding **16×12**: `Time A (nome à direita + escudo 32)` · **pílula branca central** 96px (fundo `surface`, borda `line`, raio 12, padding 4×8, texto 12/16 w500 `primary`) com **horário** (agendado) ou **placar** (ao vivo/encerrado) · `escudo 32 + Time B`. Agrupamento por data com `calendar_today_outlined` 24 + `dd/MM/yyyy` 16/24 w500.
    - Os controles (seletor + segmentos) **permanecem visíveis** em loading/erro — só a lista muda de estado.
  - **Aba Classificação** (`/competition`): **sem competição selecionada** mostra a **lista de campeonatos** (é o destino de "Ver todas"); **com competição selecionada** mostra os segmentos **Classificação** (tabela) e **Estatísticas** (líderes — mock por enquanto). Os controles de filtro **fixam a lista apenas até a primeira escolha**: ao escolher, a aba passa a mostrar a classificação.
  - **Rotas**: `/competition/:id/{games,results}` → **aba Jogos** já filtrada (segmento 0 / 2); `/competition/:id/{standings,stats}` → **aba Classificação** (segmento 0 / 1); `/competitions` → lista.
- **Public App − tela de Jogo (`GameDetailScreen`)**: hub do confronto com **duas abas** — **Estatísticas** (`KicksterStatComparison`, métricas derivadas dos lances) e **Lances** (`KicksterTimeline`); cabeçalho `KicksterGameHeader` compartilhado e **troca de aba in-place** (não empilha rotas).
  - **Regras de simplicidade (auditoria UX)**: 1 destaque por tela · **1 dado = 1 lugar** (nada aparece em card + título de seção + filtro ao mesmo tempo) · filtro só com volume · **uma taxonomia por lista** · cada estado com 1 cor/ícone, definidos só em `game_status_colors.dart` · identidade da tela aparece **uma vez**.
